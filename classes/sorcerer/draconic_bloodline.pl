...
custom_display_rule(dragon_ancestor(Color), Str) :-
    atomic_list_concat(["dragon ancestor (", Color, ")"], Str).
...
custom_display_rule(elemental_affinity(Element), Str) :-
    atomic_list_concat(["elemental affinity (", Element, ")"], Str).
