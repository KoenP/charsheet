...
breath_weapon(_) @= srd('34').

(draconic_ancestry(Color) ?= Desc) :-
    dragon_element(Color, Element),
    breath_weapon(Color, AoE, Abi),
    fmt(format_area(AoE), AoEFmt),
    format(string(Desc),
           "Gain resistance to ~w damage.
Gain a breath weapon dealing ~w damage in ~w (~w saving throw).",
           [Element, Element, AoEFmt, Abi]
          ).
