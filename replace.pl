...
% ADD NEW
options_source(match_class(Class:L), Id, Goal) :-
    replaceable_class_options(Class:L, Id, Goal).

% WANT TO REPLACE?
options_source(match_class(Class:L),
               replace(Id),
...
options(match_class(Class:L), replacing(Id, Choice), Goal) :-
    class_level(Class:CurLvl),
    replace_at_class_level(Class:CurLvl, Id, _, Goal),
    choice_member(match_class(Class:L),
                  replace(Id),
                  Choice).
...
find_choice_level(Class:Level, Id, Choice) :-
    ...
    findall(L,
            (choice_member(match_class(Class:L), ChoiceId, Choice),
